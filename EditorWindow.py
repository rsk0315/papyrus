...
##        self.status_bar.set_label('line', 'Ln: ?', side=RIGHT)
        self.status_bar.set_label('column_line', 'Ln ?, Col ?', side=RIGHT)
...
##        self.status_bar.set_label(
        self.mode_bar.set_label(
            'column_line',
##            'Ln {}, Col {}'.format(line, column)
            'L{} C{}'.format(line, column)
        )
    ...
    def set_mode_bar(self):
        ...
        self.text.bind("<<set-coding-eol>>", self.set_coding_eol)
##        self.text.bind("<<set-short-name>>", self.set_short_name)
        self.text.bind("<<set-line-and-column>>", self.set_line_and_column)
        self.text.event_add("<<set-coding-eol>>",
                            "<KeyRelease>", "<ButtonRelease>")
##        self.text.event_add("<<set-short-name>>",
##                            "<KeyRelease>", "<ButtonRelease>")
        self.text.event_add("<<set-line-and-column>>",
                            "<KeyRelease>", "<ButtonRelease>")
        self.text.after_idle(self.set_coding_eol)
    ...
    def set_coding_eol(self, event=None):
        ...
        if encoding in ('cp932',):
            coding = 'S'
        elif encoding in ('utf-8',):
            coding = 'U'
        else:  # todo
            coding = '(' + encoding + ')'
        ...
        self.mode_bar.set_label(
            'coding_eol',
            ' {}{}{}- '.format(coding, eol, modified)
        )

    def set_short_name(self, event=None):
    ...
    def createmenubar(self):
        ...
        try:
            if 'highlight' in menudict:
                ...
                menudict['highlight'].add_radiobutton(label='Whitespace', underline=0,
                    variable=self.ftype, value='Whitespace', command=self.hilite_as('.ws'))
                menudict['highlight'].add_radiobutton(label='Brainf*ck', underline=0,
                    variable=self.ftype, value='Brainf*ck', command=self.hilite_as('.bf'))
                # --------
...
##            self.mode_bar.set_label('filetype', '('+filetype+')')
    ...
    def hilite_as(self, lang):
        def makecolor():
            ...
            self.color = self.ColorDelegator(lang)
    ...
    def filename_change_hook(self):
        ...
        # print self.ext if hasattr(self, 'ext') else 'no ext'
        self.ResetColorizer()
